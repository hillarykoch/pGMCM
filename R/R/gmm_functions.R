...
fpGMM <- function(x, kmax, lambda = NULL, tol = 1e-06, itermax = 200) {
  ...
  list(
    "k" = k_out, "prop" = prop_out, "mu" = mu_out, "sigma" = sigma_out,
    "cluster" = cl_out, "BIC" = bestBIC, "lambda" = bestlam, "ll" = ll_out,
    "post_prob" = post_prob
  )
}
...
fconstr_pGMM <- function(x, lambda = NULL, tol = 1e-06, itermax = 200, penaltyType = c("SCAD", "LASSO")) {
  ...
  sigma0[sigma0 == 0] <- 1
  rho0 <- c(
    init$sigma[1, 2, ] %>% `[`(init$sigma[1, 2, ] > 0),
...
fconstr0_pGMM <- function(x, lambda = NULL, tol = 1e-06, itermax = 200) {
  # x: a matrix of data with rows for observations and columns for features
  # kmax: max number of clusters
  # lambda: a parameter of penalty term
  # d: number of replicates
  ...
  rho0_combos <- rep(0, kmax) %>%
    ...
    replace(apply(combos, 1, sum) == 0 &
      !apply(combos, 1, function(X) any(X == 0)), rho0[2]) %>%
    replace(apply(combos, 1, sum) == 2, rho0[3])
  Sigma0 <- lapply(1:nrow(sigma0), function(X) diag(sigma0[X, ]))
  ...
  list(
    "k" = k_out, "prop" = prop_out, "mu" = mu_out, "Sigma" = Sigma_out, "df" = df_out,
