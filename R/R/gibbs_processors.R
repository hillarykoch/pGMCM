...
var_traceplot <- function(param.tr, class_num, dim, ...) {
    plot(purrr::map(param.tr, "NIW") %>%
             purrr::map(class_num) %>%
             purrr::map("Sigma") %>%
             purrr::map(function(X) X[dim,dim]) %>%
             unlist,
         type = 'l',
         xlab = "iteration",
...
rho_traceplot <- function(param.tr, class_num, dim1, dim2, ...) {
    plot(purrr::map(param.tr, "NIW") %>%
             purrr::map(class_num) %>%
             purrr::map("Sigma") %>%
             purrr::map(function(X) X[dim1,dim2]) %>%
             unlist,
         type = 'l',
         xlab = "iteration",
...
mixprop_traceplot <- function(param.tr, class_num = "all", ...) {
    if(class_num == "all") {
        ...
        plot(purrr::map(param.tr, "mix_prop") %>%
             ...
             ...)
        for(i in 2:nclass) {
            lines(purrr::map(param.tr, "mix_prop") %>%
                      purrr::map(i) %>%
                      unlist,
                  type = 'l',
                  ylim = c(0,1),
...
mean_post_density_plot <- function(param.tr, class_num, dim, ...) {
    plot(density(purrr::map(param.tr, "NIW") %>%
                     purrr::map(class_num) %>%
                     purrr::map("mu") %>%
                     purrr::map(dim) %>%
                     unlist),
...
rho_post_density_plot <- function(param.tr, class_num, dim1, dim2, ...) {
    plot(density(purrr::map(param.tr, "NIW") %>%
                     purrr::map(class_num) %>%
                     purrr::map("Sigma") %>%
                     purrr::map(function(X) X[dim1,dim2]) %>%
...
mixprop_post_density_plot <- function(param.tr, class_num, ...) {
    plot(density(purrr::map(param.tr, "mix_prop") %>%
             purrr::map(class_num) %>%
             unlist),
