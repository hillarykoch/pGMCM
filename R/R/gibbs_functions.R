...
draw_NIW <- function(x, hyp, z) {
    ...
    lapply(1:nrow(hyp$hyp$mu0), function(X)
        LaplacesDemon::rnorminvwishart(
            mu0 = (hyp$kappa[X] * hyp$hyp$mu0[X, ] + nz[X] * xbar[X, ]) / (hyp$kappa[X] + nz[X]),
...
run_gibbs <- function(x, prior_prop, fits, d, red_class, nsamp) {
    ...
    z_init <-
        sample(seq(nclass),
               ...
               prob = prior_prop)
    kappa <-
        get_kappa(z_init, nclass) # kappa = nu. get_kappa can also be used to update n^z
    hyp <-
        list("kappa" = kappa, "hyp" = get_hyperparams(fits, d, red_class))
    prop0 <-
        draw_mix_prop(alpha = prior_prop, z = z_init) # can probably update this to just pass zcoll
